...
from postprocessing import postprocess_probabilities
...
def main():
    ...
    for device in manifest['devices']:
        ...
        labels, _ = postprocess_probabilities(p, meta['threshold'], meta.get('postprocessing'))
        result = pd.DataFrame(dict(timestamp=df.timestamp, label=labels))
        if args.pump_column:
