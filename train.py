...
def main():
    ...
    for device, blocks in all_blocks.items():
        ...
        if model is not None:
            ...
            pd.DataFrame(dict(feature=cols, gain=model.feature_importance('gain'))).sort_values('gain', ascending=False).to_csv(out/'models'/f'{device}_importance.csv', index=False)
        manifest['models'][device] = dict(features=cols, threshold=threshold, threshold_source=source,
